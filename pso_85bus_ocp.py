    ...
    for trunk_bus in trunk:
        ...
        for lat_idx, lateral_root in enumerate(laterals):
            ...
            while lat_queue:
                ...
                visited.add(node)
                trunk_idx = trunk.index(trunk_bus)
                x_offset = lateral_offset * (1 if lat_idx % 2 == 0 else -1)
                positions[node] = (x_offset, -trunk_idx - lat_depth - 0.5)
                for child in sorted(children.get(node, [])):
...
def plot_bus_diagram(system_id='85', figsize=(16, 12), show=True, save_path=None):
    ...
    for node in data['nodes']:
        ...
        # Bus number positioned to the right of the circle, aligned with branch
        ax.text(x + 0.55, y, str(node['bus']), ha='left', va='center', 
